...
You can also use rust-kbuild as a library in your Rust projects:
...
for (name, value) in config {
    symbols.add_symbol(name.clone(), SymbolType::Bool);
    symbols.set_value(&name, value);
}

// Write configuration
ConfigWriter::write("output.config", &symbols)?;
```

### Generate Configuration Headers

```rust
use rust_kbuild::config::ConfigGenerator;

// Generate auto.conf for makefiles
ConfigGenerator::generate_auto_conf("auto.conf", &symbols)?;

// Generate autoconf.h for C code
